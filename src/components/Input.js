...
import React, {useState} from 'react'; /* STEP 0 inport usestate*/

export default function Input() {
  /* STEP 1 set state & set to empty string*/
  ...
  const changeInput = evt => {
    ...
    /* STEP 4 - set value of input - found inside the event object*/
    setInputValue(value)
  };
  const reset = () => {
    /* STEP 5 - set input value in state to be empty string */
    setInputValue('')
  };
  ...
  return (
    <div className='widget-input container'>
      ...
      <div>
        <input id='input' type='text' onChange={changeInput} value={inputValue} /> {/* STEP 6 - add extra prop to <input /> element*/}
